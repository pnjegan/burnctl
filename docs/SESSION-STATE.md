...
- **PROJECT-MAP Other-bucket fix** (`90548b7`, 2026-06-25) — `config.py:PROJECT_MAP`
  ...
  +541 (positives = the Other drop). Digivault gained 0 — all its source JSONL is
  deleted from disk (reprocess skips missing files, cli.py:1046); verified the 9
  residual Other paths are all MISSING (data gap, not a matching bug) — did NOT
  force a manual re-tag. Two commits (config + this doc); G5 green; backup at
  `/tmp/usage.db.bak-pre-retag-2026-06-25`. **Open follow-up:** the deleted-file
  tail (incl. all digivaul) can't be re-tagged by reprocess; needs file restore or
  a separate sanctioned re-tag-from-stored-source_path op (a code change, not this goal).
